...
public class BitMatrix implements IMatrix{
	...
	{
		return new BitMatrix(nrows<right.nrows?nrows:right.nrows,ncolumns<right.ncolumns?ncolumns:right.ncolumns)
		{
			public boolean get(int i,int j)
			{
	...
	{
		ArrayList<Integer> result=new ArrayList<Integer>(nrows);
		for(int i=0;i<nrows;i++)
		{
			int count=0;
			for(int j=0;j<ncolumns;j++)
