...
public class UpperTriangularBitMatrix extends BitMatrix{
	...
	{
		return new UpperTriangularBitMatrix(Math.min(nrows, right.nrows), Math.min(ncolumns, right.ncolumns))
		{
	...
	{
		return new UpperTriangularBitMatrix(Math.max(nrows, right.nrows), Math.max(ncolumns, right.ncolumns))
				{
					public boolean get(int i,int j)
					{
						return UpperTriangularBitMatrix.this.get(i, j)||right.get(i, j);
